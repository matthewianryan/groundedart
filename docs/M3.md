...
## Audit log (append-only, capture events)
- Capture events are written to `capture_events` with an `event_type` covering creation and transitions:
  - Storage model: `apps/api/src/groundedart_api/db/models.py` (`CaptureEvent`)
  - Migration: `apps/api/src/groundedart_api/db/migrations/versions/20260123_0008_capture_events.py`
- The transition helper `apply_capture_transition_with_audit(...)` in `apps/api/src/groundedart_api/domain/capture_events.py` is used by:
...
- Capture creation records `event_type="capture_created"` in `apps/api/src/groundedart_api/api/routers/captures.py`.
...
- Admin review remains API-only; see `docs/COMMANDS.md` for CLI scripts.
...
- **Admin auth upgrade**: shared `X-Admin-Token` should be replaced by admin users/sessions before any web UI.
