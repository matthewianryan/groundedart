...
- Gating functions raise `AppError` with stable codes and structured details:
  - Rank gating: `insufficient_rank` (403) with `current_rank`, `required_rank`, and tier metadata.
  - Rate limiting: `checkin_challenge_rate_limited` (429) and `capture_rate_limited` (429) with tier + window details.
- Discovery “locked node” behavior avoids leaking restricted nodes by returning `node_not_found` (404) when rank is insufficient.
...
- `GET /v1/nodes/{id}` returns 404 for insufficient rank via `assert_can_view_node(...)`.
- `GET /v1/nodes/{id}/captures` also enforces node visibility gating before returning verified captures.
...
- **Dual rank sources**: `curator_profiles.rank` still exists and is initialized on session creation (`apps/api/src/groundedart_api/api/routers/sessions.py`), but the implemented gating uses derived rank from `rank_events`. This duplication risks drift/confusion unless explicitly deprecated or removed.
- **Projection cost**: `compute_rank_projection(...)` queries events and applies caps at read time. This is correct for MVP but may need caching/materialization as event volume grows.
- **Idempotency key extensibility**: unique `(event_type, capture_id)` works for `capture_verified`, but future rank events without `capture_id` will need a more general uniqueness strategy (e.g., `(event_type, source_kind, source_id)`).
- **Check-in UX gaps**: the web check-in flow does not yet surface rank-related API failures (e.g. `insufficient_rank`, `checkin_challenge_rate_limited`, `capture_rate_limited`) even though the API can return them.
- **Leakage posture mismatch to confirm**: discovery/detail uses “not found” for insufficient rank (404) while write paths return explicit `insufficient_rank` (403) with requirement details. This is a reasonable security posture, but it should be considered an explicit product decision.
