...
- A dedicated capture route (`/capture`) hosts the camera-first flow, wired in `apps/web/src/App.tsx`.
...
- The flow requires a valid check-in token and node context passed via navigation state from the map. It supports retake, cancel, and retry actions at each stage.
...
- `apps/api/src/groundedart_api/api/routers/captures.py` creates captures with `pending_verification` state and handles uploads with ownership checks.
