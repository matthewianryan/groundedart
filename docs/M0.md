...
### Infra (`infra`)
- `infra/docker-compose.yml` runs `postgis/postgis:16-3.4` for local development.
...
- `POST /v1/captures` (requires valid, unused check-in token; creates `pending_verification` capture)
...
- `curator_profiles`: current `rank` (used for gating)
...
- **Geofence model is “point + radius” (for now)**: check-in uses `ST_DWithin` against the node center/radius. Adding polygons later is a schema + query change.
...
- Rank computation from verified events (current rank is stored, not derived)
